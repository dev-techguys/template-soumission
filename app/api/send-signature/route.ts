...
export async function POST(request: Request) {
  try {
    ...
    // Convert base64 signature to buffer for attachment
...
<head>
  ...
  <style>
    ...
    .contract-box h3 { color: #387B84; margin: 0 0 20px; font-size: 18px; display: flex; align-items: center; gap: 10px; }
    ...
    .cta { display: inline-block; background: #F6A878; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600; margin: 20px 0; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <p>Nous vous confirmons la signature de votre contrat de banque d'heures avec <strong>TechGuys & Omnigo</strong>.</p>
      ...
      <div class="contract-box">
        ...
        ${monthlySaving > 0 ? `
        <div class="contract-row">
          <span class="contract-label">Économie mensuelle</span>
          <span class="contract-value saving">-${monthlySaving}$/mois</span>
        </div>
        ` : ''}
      </div>
      ...
      <div class="signature-section">
        ...
        <p style="margin: 0 0 15px; color: #6b7280; font-size: 14px;">Signée le ${signatureDate} à ${signatureTime}</p>
        ${signatureData ? `<img src="cid:signature" alt="Signature" class="signature-img" />` : '<p>Signature enregistrée</p>'}
      </div>

      <p style="margin-top: 30px;">Notre équipe vous contactera sous peu pour planifier le démarrage de votre accompagnement.</p>
      
      <p>En cas de questions, n'hésitez pas à nous contacter.</p>
      
      <p style="margin-top: 30px;">
        Cordialement,<br>
        <strong>L'équipe TechGuys & Omnigo</strong>
      </p>
    </div>
    <div class="footer">
      <p>Ce courriel confirme votre signature électronique.<br>
      Les documents contractuels (Offre de services et MSA) sont joints à ce courriel.</p>
    </div>
  </div>
</body>
</html>
    ...
    // HTML email for TechGuys notification
  ...
  <div class="container">
    ...
    <div class="content">
      <div class="client-info">
        ...
        <p style="margin: 5px 0;"><strong>Date de signature:</strong> ${signatureDate} à ${signatureTime}</p>
      </div>
      ...
      <div class="contract-box">
        ...
        <div class="contract-row">
          <span class="contract-label">Économie mensuelle</span>
          <span class="contract-value" style="color: #22c55e;">-${monthlySaving}$/mois</span>
        </div>
        ` : ''}
      </div>
    ...
    const techguysEmail = 'danyk@techguys.consulting'
    ...
    // Send notification email to TechGuys
    ...
    if (techguysEmailResult.error) {
      throw new Error(`TechGuys email: ${techguysEmailResult.error.message}`)
    }
